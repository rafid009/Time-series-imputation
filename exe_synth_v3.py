...
num_seasons = 50
noise = False
train_loader, valid_loader, mean, std = get_dataloader(n_steps, n_features, num_seasons, batch_size=32, missing_ratio=0.1, seed=400, is_test=False, v2='v3', noise=noise, is_mcar=False, is_col_miss=None)

model_csdi = CSDI_Synth(config_dict_csdi, device, target_dim=len(given_features)).to(device)
model_folder = "./saved_model_synth_v3"
filename = f"model_csdi_synth_v3_{miss_type_csdi}.pth"
...
mse_folder = f"results_synth_v3_{name}_all_new/metric"
data_folder = f"results_synth_v3_{name}_all_new/data"
...
evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v3', batch_size=32, mean=mean, std=std, partial_bm_config=partial_bm_config)
