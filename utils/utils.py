...
def evaluate_imputation_all(models, mse_folder, dataset_name='', batch_size=16, trials=10, length=-1, random_trial=False, forecasting=False, missing_ratio=0.01, test_indices=None, data=False, noise=False, filename='data/Daily/data_yy.npy', is_yearly=True, n_steps=366, pattern=None, mean=None, std=None):  
    ...
    results_mse = {'csdi': 0, 'diffsaits': 0, 'diffsaits_median': 0, 'diffsaits_mean_med': 0, 'saits': 0}
    results_mae = {'csdi': 0, 'diffsaits': 0, 'diffsaits_median': 0, 'saits': 0}
    results_crps = {
        'csdi_trials':{}, 'csdi': 0, 
        'diffsaits_trials': {}, 'diffsaits': 0, 'diffsaits_mean_med': 0,
    ...
    for trial in range(trials):
        ...
        for j, test_batch in enumerate(test_loader, start=1):
            ...
            if data:
                ...
                if 'DiffSAITS' in models.keys():
                    ...
                    mse_diff_saits_mean_med = ((samples_diff_saits_mean_med - c_target) * eval_points) ** 2
                    mse_diff_saits_mean_med = mse_diff_saits_mean_med.sum().item() / eval_points.sum().item()
                    diffsaits_mean_med_avg += mse_diff_saits_mean_med
        ...
        if not data:
            ...
            if 'DiffSAITS' in models.keys():
                ...
                results_mse['diffsaits_mean_med'] += diffsaits_mean_med_avg / batch_size
