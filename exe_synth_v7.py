...
import matplotlib
import pickle
from datasets.synthetic_data import feats_v7
import json
...
num_seasons = 50
noise = False
train_loader, valid_loader, mean, std = get_dataloader(n_steps, n_features, num_seasons, batch_size=16, missing_ratio=0.1, seed=seed, is_test=False, v2='v7', noise=noise)

model_csdi = CSDI_Synth(config_dict_csdi, device, target_dim=len(given_features)).to(device)
model_folder = "./saved_model_synth_v7"
filename = f"model_csdi_synth_v7_{miss_type_csdi}.pth"
...
filename = f"model_diffsaits_synth_v7_{name}_new.pth"
...
mse_folder = f"results_synth_v7_{name}_all_new/metric"
data_folder = f"results_synth_v7_{name}_all_new/data"

# evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v7', batch_size=32, mean=mean, std=std, partial_bm_config=partial_bm_config)
...
for l in lengths:
    ...
    # evaluate_imputation_all(models=models, mse_folder=data_folder, dataset_name='synth_v7', length=l, trials=1, batch_size=1, data=True, noise=noise)

print(f"\nForecasting:")
evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v7', batch_size=32, length=(10, 80), forecasting=True, noise=noise, mean=mean, std=std)
# evaluate_imputation_all(models=models, mse_folder=data_folder, forecasting=True, dataset_name='synth_v7', length=l, trials=1, batch_size=1, data=True, noise=noise)
...
for ratio in miss_ratios:
    ...
    # evaluate_imputation_all(models=models, mse_folder=data_folder, dataset_name='synth_v7', trials=1, batch_size=1, data=True, missing_ratio=ratio, random_trial=True, noise=noise)
