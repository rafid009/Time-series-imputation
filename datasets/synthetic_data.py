...
feats_v5 = ['sin', 'cos', 'sin-cos-hist-plus', 'sin-cos-hist-prod']
feats_v6 = ['sin', 'sin2', 'cos', 'cos2']
...
def create_synthetic_data_v5(n_steps, num_seasons, seed=10, rate=0.05, length_rate=0.02, noise=False):
    ...
    for i in range(data.shape[0]):
        for feature in synth_features.keys():
            ...
            elif feature == 'sin-cos-hist-plus':
                data[i, 0, feats_v5.index(feature)] = data[i, 0, feats_v5.index('sin')]
                f1_sin = data[i, 1:, feats_v5.index('sin')]
                f2_cos = data[i, 1:, feats_v5.index('cos')]
                data[i, 1:, feats_v5.index(feature)] = f1_sin + f2_cos
            elif feature == 'sin-cos-hist-prod':
                data[i, 0, feats_v5.index(feature)] = data[i, 0, feats_v5.index('cos')]
                f1_sin = data[i, 1:, feats_v5.index('sin')]
                f2_cos = data[i, 1:, feats_v5.index('cos')]
                data[i, 1:, feats_v5.index(feature)] = f1_sin * f2_cos
            if noise and (feature != 'sin-cos-hist-plus' or feature != 'sin-cos-hist-prod'):
                noise_level = np.random.rand(data.shape[1]) * 0.5
                data[i, :, feats_v5.index(feature)] += noise_level
    ...
    for i in range(data.shape[0]):
        for feature in synth_features.keys():
            ...
            if feature == 'sin':
                ...
                f1_sin = data[i, :, feats_v5.index('sin')]
                data[i, :, feats_v5.index(feature)] = 1 - f1_sin 
            elif feature == 'cos2':
                f2_cos = data[i, :, feats_v5.index('cos')]
                data[i, :, feats_v5.index(feature)] = 1 - 5 * f2_cos
            if noise and (feature != 'sin2' or feature != 'cos2'):
                noise_level = np.random.rand(data.shape[1]) * 0.5
                data[i, :, feats_v5.index(feature)] += noise_level
