...
import matplotlib
import pickle
from datasets.synthetic_data import create_synthetic_data_v6, feats_v6
import json
...
given_features = feats_v6 #['sin', 'cos2', 'harmonic', 'weight', 'lin_comb', 'non_lin_comb', 'mixed_history']
...
filename = f"model_csdi_synth_v6{'_noise' if noise else ''}.pth"
...
saits_model_file = f"{model_folder}/saits_model_synth_v6{'_noise' if noise else ''}.pkl"
...
X, mean, std = create_synthetic_data_v6(n_steps, num_seasons, seed=10, noise=noise)
...
filename = f"model_diffsaits_synth_v6_qual{'_noise' if noise else ''}.pth"
...
mse_folder = f"results_synth_v6_qual{'_noise' if noise else ''}"
data_folder = f"results_synth_v6_qual{'_noise' if noise else ''}"
lengths = [10, 50, 90]
for l in lengths:
    print(f"\nlength = {l}")
    print(f"\nBlackout:")
    evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v6', batch_size=32, length=l, noise=noise)
...
print(f"\nForecasting:")
evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v6', batch_size=32, length=(10, 80), forecasting=True, noise=noise)
...
for ratio in miss_ratios:
    ...
    evaluate_imputation_all(models=models, trials=10, mse_folder=mse_folder, dataset_name='synth_v6', batch_size=32, missing_ratio=ratio, random_trial=True, noise=noise)
